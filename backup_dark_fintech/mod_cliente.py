...
def mostrar(df, casos, cfg):
    st.markdown("""<div class="info-box"><strong>ANÁLISIS POR CLIENTE</strong> — Perfil detallado de comportamiento transaccional Sentinel. Identifica vectores de riesgo individuales, picos de actividad estadística y patrones de fragmentación técnica (smurfing).</div>""", unsafe_allow_html=True)
    ...
    if "Crítico" in nivel_c:
        resumen_manual += "🔴 <strong>Recomendación:</strong> Congelar transacciones inmediatamente y reportar este caso a la Unidad de Cumplimiento e UIF/IVE mediante un reporte de transacción sospechosa."
    elif "Alto" in nivel_c:
        resumen_manual += "🟠 <strong>Recomendación:</strong> Solicitar Debida Diligencia Ampliada (EDD) e indagar activamente por los documentos de justificación del origen de los fondos."
    elif "Medio" in nivel_c:
        resumen_manual += "🟡 <strong>Recomendación:</strong> Proceder a clasificación bajo monitoreo reforzado y programar una actualización inminente de su perfil legal y financiero."
    else:
        resumen_manual += "🟢 <strong>Recomendación:</strong> La actividad se considera dentro de los límites y perfil del usuario, continuar bajo las medidas de Debida Diligencia Estándar vigente."
    ...
    <div style="background-color: #1b2027; border: 1px solid {nivel_color}; border-radius: 0px; padding: 24px; border-left: 8px solid {nivel_color};">
        <div style="font-size: 11px; color: #f59e0b; text-transform: uppercase; letter-spacing: 2px; font-family: 'IBM Plex Mono', monospace; margin-bottom: 15px;">
            <span class="pulse-dot"></span> RESUMEN TÉCNICO SENTINEL INTELLIGENCE
        </div>
