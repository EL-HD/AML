...
def mostrar(casos, matriz_alertas):
    st.markdown("""<div class="info-box"><strong>MATRICES DE RIESGO</strong> — Arquitectura de decisión Sentinel. Clasificación técnica por perfiles de riesgo y tipologías de alerta analitica. Optimizado para calibración de umbrales y priorización táctica.</div>""", unsafe_allow_html=True)

    st.markdown('<div class="section-title">👥 Matriz de Riesgo por Cliente</div>', unsafe_allow_html=True)
    st.markdown("""<div class="glossary"><div class="glossary-title">📖 DICCIONARIO DE DATOS</div><div class="glossary-item"><span class="glossary-key">Cliente</span><span>Identificador soberano.</span></div><div class="glossary-item"><span class="glossary-key">Total_Mensual</span><span>Volumen económico en el ciclo.</span></div><div class="glossary-item"><span class="glossary-key">Score_Max</span><span>Magnitud de riesgo (Sentinel 0–12).</span></div><div class="glossary-item"><span class="glossary-key">Transacciones</span><span>Densidad operativa.</span></div><div class="glossary-item"><span class="glossary-key">Nivel_Riesgo</span><span>Clasificación táctica.</span></div></div>""", unsafe_allow_html=True)

    st.dataframe(
        casos.sort_values("Score_Max", ascending=False).reset_index(drop=True),
    ...
    # Gráfica de pesos
    st.markdown("---")
    st.markdown('<div class="section-title">⚖️ Contribución al Score por Tipo de Alerta</div>', unsafe_allow_html=True)
