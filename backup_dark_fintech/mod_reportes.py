...
# ── Paleta y helpers (Sovereign Intelligence) ────────────────────────
...
C_TEXT      = rl_colors.HexColor("#dee2ed")   
C_MUTED     = rl_colors.HexColor("#a08e7a")   
C_WHITE     = rl_colors.HexColor("#dee2ed")
...
def make_styles():
    ...
    S["tbl_cell"]  = ParagraphStyle("tbl_cell",  fontSize=8, textColor=rl_colors.HexColor("#e6edf3"), fontName="Helvetica", alignment=TA_CENTER)
    S["tbl_cell_l"]= ParagraphStyle("tbl_cell_l",fontSize=8, textColor=rl_colors.HexColor("#e6edf3"), fontName="Helvetica", alignment=TA_LEFT)
    return S
...
def df_to_table(df_t: pd.DataFrame, col_widths=None, zebra=True) -> Table:
    ...
    style_cmds = [
        ...
        ("ROWBACKGROUNDS",(0,1), (-1,-1), [C_PANEL, C_BG] if zebra else [C_PANEL]),
...
def page_header_footer(canvas_obj, doc, report_title, fecha_str):
    ...
    canvas_obj.setFillColor(C_AMBER)
    canvas_obj.rect(0, H - 37, W, 1.5, fill=1, stroke=0)
    canvas_obj.setFont("Helvetica-Bold", 9)
    canvas_obj.setFillColor(C_WHITE)
    canvas_obj.drawString(1.0 * inch, H - 24, "AML Intelligence Platform")
    ...
    canvas_obj.drawCentredString(W / 2, 10, "Ing. Hob\u00e9d D\u00edaz, Msc. M.A.F.I")
...
def mostrar(df, casos, matriz_alertas, cfg):
    ...
    def generar_reporte_cliente(cliente_sel: str) -> bytes:
        ...
        cover_data = [[
            Table([
                [Paragraph("FICHA DE INVESTIGACI\u00d3N AML", ParagraphStyle("ct",
        ...
        story.append(kpi_row([
            ...
            ("Score AML",         f"{score_c}/{score_max_teorico}", C_AMBER),
            ("Total Transaccionado", f"Q{total_c:,.0f}",    C_BLUE),
            ("Num. Operaciones",  str(tx_c),                C_TEXT),
        ]))
        story.append(Spacer(1, 14))

        C_ROW_EVEN = rl_colors.HexColor("#161b22")
        C_ROW_ALT  = rl_colors.HexColor("#1e2535")
        C_TXT_W    = rl_colors.HexColor("#e6edf3")
        ...
        for nombre_a, activa_a, param_a, peso_a in alertas_info:
            ...
            estado_p  = Paragraph("ACTIVA", ParagraphStyle("ae", fontSize=8,
                        ...
                        if activa_a else \
                        Paragraph("OK", ParagraphStyle("ao", fontSize=8,
        ...
        stat_data = [
            ...
            [Paragraph("N. operaciones", S_CELL_L),
             ...
             Paragraph(str(picos_cnt), ParagraphStyle("ps", fontSize=8,
                 ...
                 fontName="Helvetica-Bold", alignment=TA_CENTER))],
            [Paragraph("Score AML maximo", S_CELL_L),
        ...
        if "Cr\u00edtico" in nivel_c:
            concl = (f"El cliente <b>{cliente_sel}</b> presenta un nivel de riesgo <b>CRITICO</b> "
                     ...
                     f"documentar el expediente y evaluar la presentacion de un Reporte de "
                     f"Operacion Sospechosa (ROS) ante la Unidad de Inteligencia Financiera (UIF).")
            accion = "ACCION INMEDIATA: Bloquear operaciones, notificar al Oficial de Cumplimiento y activar protocolo ROS."
            accion_color = C_RED
            ...
            concl = (f"El cliente <b>{cliente_sel}</b> no presenta alertas significativas en "
                     f"el periodo analizado. Score AML: {score_c}/{score_max_teorico}.")
        ...
        story.append(Paragraph(
            f"Reporte generado automaticamente por AML Intelligence Platform v2.0 \u00b7 "
    ...
    def generar_informe_general() -> bytes:
        ...
        def _hf(c, d):
            page_header_footer(c, d, "Informe Ejecutivo AML", fecha_gen)
        ...
        portada = Table([[
            Table([
                [Paragraph("INFORME EJECUTIVO", ParagraphStyle("pt",
                    ...
                    alignment=TA_CENTER))],
                [Paragraph("Sistema Anti-Lavado de Activos", ParagraphStyle("ps",
                    fontSize=20, textColor=C_WHITE, fontName="Helvetica-Bold",
                    alignment=TA_CENTER))],
                [Paragraph("AML Intelligence Platform", ParagraphStyle("pp",
        ...
        plt.tight_layout()
        desc_pie = "<b>\ud83d\udccc Interpretacion:</b> Agrupacion porcentual del total de clientes evaluados segun su categorizacion de riesgo final calculada por AML. Util para entender la exposicion general de la cartera."
        ...
        sintesis = (
            ...
            f"<b>Q{monto_total_g:,.0f}</b>. El sistema AML genero <b>{total_alertas_g:,} alertas</b> "
        ...
        story.append(Spacer(1, 8))
        story.append(Paragraph(
            f"Informe generado automaticamente por AML Intelligence Platform v2.0 \u00b7 "
    ...
    with tab_ind:
        ...
        if cliente_pdf:
            ...
            with col_p2:
                st.markdown(f"""<div class="metric-card amber">
                    ...
                    <div class="metric-label">Score AML</div></div>""",
                    unsafe_allow_html=True)
            with col_p3:
        ...
        if generar_ind:
            with st.spinner(f"\u2699\ufe0f Generando ficha de investigaci\u00f3n para {cliente_pdf}..."):
                try:
    ...
    with tab_gen:
        ...
        if generar_gen:
            with st.spinner("\u2699\ufe0f Compilando informe ejecutivo general..."):
                try:
