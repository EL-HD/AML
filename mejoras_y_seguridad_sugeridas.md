...
4. **Almacenamiento Persistente Real**: Migrar del estado local en memoria (`st.session_state`) a una base de datos ligera como SQLite o PostgreSQL (mediante *SQLAlchemy*), lo que permitiría que múltiples inspectores colaboren sobre el mismo set de transacciones y se preserven los historiales diarios.
...
* **A07:2021 – Identification and Authentication Failures (Fallos de Autenticación)**:
  * *Implementación Sugerida*: Forzar una política de expiro de sesión automática. En Streamlit es común que la sesión no muera hasta que se refresca el navegador; es fundamental un "Session Timeout" dinámico post-15 minutos de inactividad que purgue el estado (`st.session_state.clear()`).
* **A08:2021 – Software and Data Integrity Failures (Integridad de Software y Datos)**:
  * *Implementación Sugerida*: Implementar chequeos de integridad al subir el archivo Excel base. Validar internamente los hashes SHA-256 de librerías utilizadas para evitar ataques en la Cadena de Suministro (Supply Chain Attacks).
